...
const Chat = () => {
    ...
    useEffect(() => {
        ...
        await fetch(`https://stage.backend.brexa.xyz/room/${name}/${password}`, {
            method: 'GET',
            headers: {
    ...
    const Send = async (e) => {
        ...
        await axios(`https://stage.backend.brexa.xyz/room/${name}/${password}`, {
            method: 'POST',
            headers: {
                "Authorization": `Bearer ${authTokens.access}`,
                ...
                <div className="message" id='messagesContainer'>
                    {messages && messages.map((message) => {
                        return <div className={`${user.username == message.user ? 'owner' : 'another'}`} key={message.id}><h3>{message.user}</h3> <p>{message.message}</p>
                            {message.image ? <img className='image' style={{'width': 'auto', 'height': "100px", "display": "block"}} src={`https://stage.backend.brexa.xyz${message.image}`} loading="lazy" width={300} height={150} /> : ''}
                        </div>
                    })}
                </div>
            </div>
